...
def init_parameters():
    global devices, sched_headers ,days, tol,device_state, start_time,bg_window,devices_headers
...
def device_chage_state(dev,stat,task="",text=[""]):
    ...
    if stat != device_state[dev]: #mainsw_state.get()=="on" and           #Mainswtich is ON
    ...
    #if device_state[dev]== "on": #mainsw_state.get()=="off" and         #MainSwitch shutdown
...
def create_GUI():
    ...
    def clock():
        ...
        clock_label.config(text=time+" Uptime: "+uptime(start_time))
...
def schedule_loop():
    ...
    for i in range(1,tasks_total+1):                                     # is task off ?
        ...
        if str(days[today]) in sch_file[i][2] and "on" in sch_file[i][1]: 
            ...
            if isin_stop[1] >0 and isin_start[1] < 0  :                 ## inside On interval
                if continue_task[i-1] == 'off':## or buts.status[int(sch_file [i][5])-1][0].get()=="on"  
                    ##label_def_onoff(i,0)
                    continue_task[i-1] = 'off'
                if continue_task[i-1] !="off": ##buts.status[int(sch_file [i][5])-1][0].get() == "off" and 
